class LinkedBinaryTree(BinaryTree):
    ...
    def num_children(self, p):
        """Return the number if children of position p"""
        node = self._validate(p)
        count = 0
    ...
    def _add_left(self, p, e):
        ...
        Raise ValueError if position p i invalid or p already has a left child
        """
    ...
    def _add_right(self, p, e):
        ...
        node = self._validate(p)
        if node._right is not None: raise ValueError('Riight child exists')
        self._size += 1
